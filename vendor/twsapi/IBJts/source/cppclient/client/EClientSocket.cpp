...
static void SocketCloseAndInvalidate(std::atomic<SOCKET>& fd)
{
  SOCKET old = fd.exchange(INVALID_SOCKET);
  if (SocketValid(old))
    SocketClose(old);
}
