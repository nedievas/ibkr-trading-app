...
Decimal DecimalFunctions::add(Decimal decimal1, Decimal decimal2) {
    unsigned int flags;
    return Decimal::fromBits(__bid64_add(decimal1.bits(), decimal2.bits(), 0, &flags));
}

Decimal DecimalFunctions::sub(Decimal decimal1, Decimal decimal2) {
    unsigned int flags;
    return Decimal::fromBits(__bid64_sub(decimal1.bits(), decimal2.bits(), 0, &flags));
}

Decimal DecimalFunctions::mul(Decimal decimal1, Decimal decimal2) {
    unsigned int flags;
    return Decimal::fromBits(__bid64_mul(decimal1.bits(), decimal2.bits(), 0, &flags));
}

Decimal DecimalFunctions::div(Decimal decimal1, Decimal decimal2) {
    unsigned int flags;
    return Decimal::fromBits(__bid64_div(decimal1.bits(), decimal2.bits(), 0, &flags));
}

double DecimalFunctions::decimalToDouble(Decimal decimal) {
    unsigned int flags;
    return __bid64_to_binary64(decimal.bits(), 0, &flags);
}

Decimal DecimalFunctions::doubleToDecimal(double d) {
    unsigned int flags;
    return Decimal::fromBits(__binary64_to_bid64(d, 0, &flags));
}
...
Decimal DecimalFunctions::stringToDecimal(std::string str) {
    ...
    return Decimal::fromBits(__bid64_from_string(const_cast<char*>(str.c_str()), 0, &flags));
}

std::string DecimalFunctions::decimalToString(Decimal value) {
    char buf[64];
    unsigned int flags;
    __bid64_to_string(buf, value.bits(), &flags); // convert Decimal value to string using bid64_to_string function
    return buf;
}
