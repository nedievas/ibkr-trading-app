...
std::unique_ptr<DeltaNeutralContract> EDecoderUtils::decodeDeltaNeutralContract(const protobuf::Contract& contractProto) {
    std::unique_ptr<DeltaNeutralContract> deltaNeutralContract;
    if (contractProto.has_deltaneutralcontract()) {
        deltaNeutralContract.reset(new DeltaNeutralContract());
...
Order EDecoderUtils::decodeOrder(int orderId, const protobuf::Contract& contractProto, const protobuf::Order& orderProto) {
    ...
    if (orderProto.has_hedgetype() && orderProto.has_hedgemaxsize()) order.hedgeMaxSize = orderProto.hedgemaxsize();
...
SoftDollarTier EDecoderUtils::decodeSoftDollarTier(const protobuf::SoftDollarTier& softDollarTierProto) {
    ...
    const std::string& name = softDollarTierProto.name();
    const std::string& value = softDollarTierProto.value();
    const std::string& displayName = softDollarTierProto.displayname();
...
ContractDetails EDecoderUtils::decodeContractDetails(const protobuf::Contract& contractProto, const protobuf::ContractDetails& contractDetailsProto, bool isBond) {
    ...
    if (contractDetailsProto.has_settlementmethod()) contractDetails.settlementMethod = contractDetailsProto.settlementmethod();
...
SmartComponentsMap EDecoderUtils::decodeSmartComponents(const protobuf::SmartComponents& smartComponentsProto) {
    ...
    if (smartComponentsCount > 0) {
        for (int i = 0; i < smartComponentsCount; ++i) {
            ...
            const std::string& exchange = smartComponentProto.exchange();
